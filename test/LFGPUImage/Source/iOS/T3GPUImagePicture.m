...
@implementation T3GPUImagePicture
...
- (id)initWithCGImage:(CGImageRef)newImageSource smoothlyScaleOutput:(BOOL)smoothlyScaleOutput removePremultiplication:(BOOL)removePremultiplication;
{
    ...
    CGSize scaledImageSizeToFitOnGPU = [T3GPUImageContext sizeThatFitsWithinATextureForSize:pixelSizeOfImage];
    ...
    LFrunSynchronouslyOnVideoProcessingQueue(^{
        [T3GPUImageContext useImageProcessingContext];
        
        outputFramebuffer = [[T3GPUImageContext sharedFramebufferCache] fetchFramebufferForSize:pixelSizeToUseForTexture onlyTexture:YES];
...
- (BOOL)processImageWithCompletionHandler:(void (^)(void))completion;
{
    ...
    LFrunAsynchronouslyOnVideoProcessingQueue(^{        
        for (id<LFGPUImageInput> currentTarget in targets)
        {
...
- (void)processImageUpToFilter:(T3GPUImageOutput<LFGPUImageInput> *)finalFilterInChain withCompletionHandler:(void (^)(UIImage *processedImage))block;
{
...
- (void)addTarget:(id<LFGPUImageInput>)newTarget atTextureLocation:(NSInteger)textureLocation;
{
