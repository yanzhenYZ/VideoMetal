...
{
    dispatch_queue_t videoProcessingQueue = [T3GPUImageContext sharedContextQueue];
...
#else
	if (dispatch_get_specific([T3GPUImageContext contextKey]))
#endif
	{
		block();
	}else
...
@implementation T3GPUImageOutput
...
- (void)removeTarget:(id<T3GPUImageInput>)targetToRemove;
{
    ...
    T3runSynchronouslyOnVideoProcessingQueue(^{
		...
		[targetToRemove setInputRotation:kT3GPUImageNoRotation atIndex:textureIndexOfTarget];
...
- (void)removeAllTargets;
{
    cachedMaximumOutputSize = CGSizeZero;
    T3runSynchronouslyOnVideoProcessingQueue(^{
        for (id<T3GPUImageInput> targetToRemove in targets)
        {
            NSInteger indexOfObject = [targets indexOfObject:targetToRemove];
            NSInteger textureIndexOfTarget = [[targetTextureIndices objectAtIndex:indexOfObject] integerValue];
...
{
    T3GPUImagePicture *stillImageSource = [[T3GPUImagePicture alloc] initWithCGImage:imageToFilter];
    
    [self useNextFrameForImageCapture];
    [stillImageSource addTarget:(id<T3GPUImageInput>)self];
    [stillImageSource processImage];
    
    CGImageRef processedImage = [self newCGImageFromCurrentlyProcessedOutput];
    
    [stillImageSource removeTarget:(id<T3GPUImageInput>)self];
...
-(void)setOutputTextureOptions:(LFGPUTextureOptions)outputTextureOptions
{
