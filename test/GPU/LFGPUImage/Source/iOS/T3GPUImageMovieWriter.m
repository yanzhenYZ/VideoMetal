...
NSString *const kT3GPUImageColorSwizzlingFragmentShaderString = SHADER_STRING
(
...
@interface T3GPUImageMovieWriter ()
{
    GLuint movieFramebuffer, movieRenderbuffer;
    
    T3GLProgram *colorSwizzlingProgram;
...
- (void)renderAtInternalSizeUsingFramebuffer:(T3GPUImageFramebuffer *)inputFramebufferToUse;

@end

@implementation T3GPUImageMovieWriter
...
- (id)initWithMovieURL:(NSURL *)newMovieURL size:(CGSize)newSize fileType:(NSString *)newFileType outputSettings:(NSMutableDictionary *)outputSettings;
{
    ...
    _movieWriterContext = [[T3GPUImageContext alloc] init];
    [_movieWriterContext useSharegroup:[[[T3GPUImageContext sharedImageProcessingContext] context] sharegroup]];

    T3runSynchronouslyOnContextQueue(_movieWriterContext, ^{
        [_movieWriterContext useAsCurrentContext];
        
        if ([T3GPUImageContext supportsFastTextureUpload])
        {
            colorSwizzlingProgram = [_movieWriterContext programForVertexShaderString:kT3GPUImageVertexShaderString fragmentShaderString:kT3GPUImagePassthroughFragmentShaderString];
        }
        else
        {
            colorSwizzlingProgram = [_movieWriterContext programForVertexShaderString:kT3GPUImageVertexShaderString fragmentShaderString:kT3GPUImageColorSwizzlingFragmentShaderString];
        }
...
- (void)finishRecordingWithCompletionHandler:(void (^)(void))handler;
{
    T3runSynchronouslyOnContextQueue(_movieWriterContext, ^{
        ...
        {
            if (handler)
                T3runAsynchronouslyOnContextQueue(_movieWriterContext, handler);
            return;
        }
        if( assetWriter.status == AVAssetWriterStatusWriting && ! videoEncodingIsFinished )
        {
...
//        T3runAsynchronouslyOnContextQueue(_movieWriterContext, write);
...
- (void)enableSynchronizationCallbacks;
{
    if (videoInputReadyCallback != NULL)
    {
        ...
        videoQueue = dispatch_queue_create("com.sunsetlakesoftware.GPUImage.videoReadingQueue", T3GPUImageDefaultQueueAttribute());
        [assetWriterVideoInput requestMediaDataWhenReadyOnQueue:videoQueue usingBlock:^{
            ...
            while( assetWriterVideoInput.readyForMoreMediaData && ! _paused )
            {
                ...
                {
                    T3runAsynchronouslyOnContextQueue(_movieWriterContext, ^{
                        if( assetWriter.status == AVAssetWriterStatusWriting && ! videoEncodingIsFinished )
                        {
                            videoEncodingIsFinished = YES;
                            [assetWriterVideoInput markAsFinished];
    ...
    if (audioInputReadyCallback != NULL)
    {
        audioQueue = dispatch_queue_create("com.sunsetlakesoftware.GPUImage.audioReadingQueue", T3GPUImageDefaultQueueAttribute());
...
- (void)renderAtInternalSizeUsingFramebuffer:(T3GPUImageFramebuffer *)inputFramebufferToUse;
{
    ...
    const GLfloat *textureCoordinates = [T3GPUImageFilter textureCoordinatesForRotation:inputRotation];
...
#pragma mark -
#pragma mark T3GPUImageInput protocol
...
- (void)newFrameReadyAtTime:(CMTime)frameTime atIndex:(NSInteger)textureIndex;
{
    ...
    T3GPUImageFramebuffer *inputFramebufferForBlock = firstInputFramebuffer;
    glFinish();
