...
@interface T3GPUImageFramebuffer()
{
    GLuint framebuffer;
...
void dataProviderReleaseCallback3T (void *info, const void *data, size_t size);
void dataProviderUnlockCallback3T (void *info, const void *data, size_t size);

@implementation T3GPUImageFramebuffer
...
- (void)generateFramebuffer;
{
    T3runSynchronouslyOnVideoProcessingQueue(^{
        ...
        {
#if TARGET_IPHONE_SIMULATOR || TARGET_OS_IPHONE
            CVOpenGLESTextureCacheRef coreVideoTextureCache = [[T3GPUImageContext sharedImageProcessingContext] coreVideoTextureCache];
...
- (void)destroyFramebuffer;
{
    T3runSynchronouslyOnVideoProcessingQueue(^{
        ...
        if ([T3GPUImageContext supportsFastTextureUpload] && (!_missingFramebuffer))
        {
#if TARGET_IPHONE_SIMULATOR || TARGET_OS_IPHONE
            if (renderTarget)
            {
...
- (void)unlock;
{
    ...
    {
        [[T3GPUImageContext sharedFramebufferCache] returnFramebufferToCache:self];
    }
}
...
void dataProviderUnlockCallback3T (void *info, const void *data, size_t size)
{
    T3GPUImageFramebuffer *framebuffer = (__bridge_transfer T3GPUImageFramebuffer*)info;
    
    [framebuffer restoreRenderTarget];
    [framebuffer unlock];
    [[T3GPUImageContext sharedFramebufferCache] removeFramebufferFromActiveImageCaptureList:framebuffer];
}
...
- (CGImageRef)newCGImageFromFramebufferContents;
{
    ...
    T3runSynchronouslyOnVideoProcessingQueue(^{
        ...
        if ([T3GPUImageContext supportsFastTextureUpload])
        {
            ...
            dataProvider = CGDataProviderCreateWithData((__bridge_retained void*)self, rawImagePixels, paddedBytesForImage, dataProviderUnlockCallback3T);
            [[T3GPUImageContext sharedFramebufferCache] addFramebufferToActiveImageCaptureList:self]; // In case the framebuffer is swapped out on the filter, need to have a strong reference to it somewhere for it to hang on while the image is in existence
#else
#endif
        }
        else
        {
            ...
            dataProvider = CGDataProviderCreateWithData(NULL, rawImagePixels, totalBytesForImage, dataProviderReleaseCallback3T);
...
- (void)unlockAfterReading
{
    ...
    if ([T3GPUImageContext supportsFastTextureUpload])
    {
        NSAssert(readLockCount > 0, @"Unbalanced call to -[T3GPUImageFramebuffer unlockAfterReading]");
        readLockCount--;
        if (readLockCount == 0)
        {
